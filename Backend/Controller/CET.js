...
export const addCETStudentResult = async (req, res) => {
    try {
        ...
        res.status(201).json({ message: "CET student result added successfully", success: true, data: savedResult });
    } catch (error) {
        console.error(error);
...
export const getAllCETStudentResults = async (req, res) => {
    try {
        ...
        res.status(200).json({ message: "CET student results fetched successfully", success: true, data: results });
    } catch (error) {
        res.status(500).json({ message: error.message, success: false });
    }
};

export const getCETResultById = async (req, res) => {
    try {
        const { id } = req.params;
        const result = await CETResult.findById(id);

        if (!result) {
            return res.status(404).json({ message: "CET student result not found", success: false });
        }

        res.status(200).json({ message: "CET student result fetched successfully", success: true, data: result });
    } catch (error) {
        res.status(500).json({ message: error.message, success: false });
    }
};
...
export const editCETStudentResult = async (req, res) => {
    try {
        ...
        const result = await CETResult.findByIdAndUpdate(
            id,
            {
                firstName,
                lastName,
        ...
        res.status(200).json({ message: "CET student result updated successfully", success: true, data: result });
    } catch (error) {
        res.status(500).json({ message: error.message, success: false });
    }
};
