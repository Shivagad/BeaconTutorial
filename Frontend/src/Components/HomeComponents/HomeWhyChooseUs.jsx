      ...
      <div className="max-w-7xl mx-auto relative">
        <div className="text-center mb-16 mt-5">
          <div className="inline-block relative">
            ...
            <Zap className="absolute -right-8 -top-1 w-6 h-6 text-yellow-400 animate-glow" />
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {features.map((feature, index) => (
            <div key={index} className="group hover-scale animate-tilt">
              <div className="h-full bg-white rounded-2xl p-8 relative overflow-hidden">
                <div
                  className={`absolute inset-0 bg-gradient-to-br ${feature.color} opacity-10 group-hover:opacity-30 transition-opacity duration-700`}
                ></div>

                <div className="relative z-10">
                  <div
                    className={`w-16 h-16 rounded-xl bg-gradient-to-br ${feature.color} p-4 mb-6`}
                  >
                  ...
                  <p className="text-gray-900 mb-6">{feature.description}</p>
                </div>
              </div>
            </div>
          ))}
